...
// newSnapshotStore wraps hashicorp/raft's FileSnapshotStore. retain=3
// keeps the last three snapshots on disk; older ones are reaped
// automatically. This matches the hashicorp/raft default.
...
// Snapshot wire format (streamed into the raft SnapshotSink):
//
//	[4]  magic       "CFAS"
...
//	    [vlen] val   raw bytes
//
// Format is internal — bumping the version field cleanly breaks
// compatibility should we ever need to reshape the on-wire layout.
const (
	snapshotMagic       = "CFAS"
	snapshotVersionV1   = uint32(1)
	snapshotTagEntry    = byte(0x01)
	snapshotTagEOF      = byte(0x00)
	snapshotStatePrefix = "cefas/" // FSM-relevant key range
	snapshotStateEnd    = "cefas0" // one past 'cefas/' (0x30 > 0x2F)
)

// writeSnapshot streams the live cefas/ keys from a pebble.Snapshot
// into w. The caller passes a snapshot (not the raw DB) so the
// iterator sees a stable point-in-time view even as new writes land.
...
// streaming new keys it wipes the existing cefas/ range so the
// follower's state ends up exactly matching the leader at snapshot
// time. The raft/ range is left alone — log and stable state belong
// to the local node, not the snapshot stream.
