...
WITH_MV_FAST="${WITH_MV_FAST:-0}"
MV_FAST_INTERVAL="${MV_FAST_INTERVAL:-5}"
...
with_mv_fast_args() {
  if [[ "$WITH_MV_FAST" == "1" ]]; then
    printf -- '-with-mv-fast\n'
    if [[ -n "${MV_FAST_INTERVAL:-}" ]]; then
      printf -- '-mv-fast-interval\n%s\n' "$MV_FAST_INTERVAL"
    fi
  fi
}
...
run_phase "smoke" "$ROUTE_BIN" \
  -nodes "$NODE_MAP" \
  $(client_route_aware_args) $(with_stream_args) $(with_plugin_index_args) $(with_mv_eager_args) $(with_mv_fast_args) \
  -table "$SMOKE_TABLE" \
  -items "$SMOKE_ITEMS" \
