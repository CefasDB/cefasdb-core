...
# tmp/cefasctl-smoke.env ou defina as variaveis CEFAS_AUTH_* no ambiente.
# Nao commite senhas nem tokens.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CEFAS_SMOKE_ENV_FILE="${CEFAS_SMOKE_ENV_FILE:-${ROOT_DIR}/tmp/cefasctl-smoke.env}"
...
build_cli_if_needed() {
  ...
  echo "+ go build -o $CEFAS_BIN ./cmd/cefasctl" >&2
  (cd "$ROOT_DIR" && go build -o "$CEFAS_BIN" ./cmd/cefasctl)
}
